...
public:
  ...
  bool isAutoClosure() const { return bool(getAutoClosure()); }

  AutoClosureExpr *getAutoClosure() const {
    if (!Function)
      return nullptr;
    ...
    return dyn_cast_or_null<AutoClosureExpr>(closure);
  }
  ...
  void diagnoseUnhandledThrowSite(DiagnosticEngine &Diags, ASTNode E,
    ...
    case Kind::PotentiallyHandled:
      ...
      if (auto *autoClosure = getAutoClosure()) {
        // Implicit synthesized autoclosure generated by the rewriter to wrap
        // `async let` should be transparent to diagnostics.
        if (autoClosure->isImplicit() &&
